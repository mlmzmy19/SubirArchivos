...
function tt_EvY(t_e)
{
	...
	if(tt_sup && (t_y2 = t_y - (tt_obj_jjH + tt_offY - 15)) >= tt_Int(window.pageYOffset || (tt_db? tt_db.scrollTop : 0) || 0))
...
{
	if(tt_obj_jj) tt_Hide();
	tt_mf = document.onmousemove || null;
	if(window.dd && (window.DRAG && tt_mf == DRAG || window.RESIZE && tt_mf == RESIZE)) return;
	var t_sh, t_h;

	tt_obj_jj = tt_GetDiv(t_id);
	if(tt_obj_jj)
	{
		t_e = t_e || window.event;
		tt_sub = !(tt_sup = t_sup);
		tt_sticky = t_sticky;
		tt_obj_jjW = tt_GetDivW();
		tt_obj_jjH = tt_GetDivH();
		...
		else
		{
			...
			if(t_sh)
			{
				t_h = tt_obj_jjH - tt_Int(t_sh.style.pixelTop || t_sh.style.top || 0);
...
function tt_Hide()
{
	if(window.tt_obj_jj)
	{
		...
		if(!tt_sticky || !tt_act)
		{
			...
			tt_ShowDiv(false);
			tt_SetDivPos(-tt_obj_jjW, -tt_obj_jjH);
			tt_obj_jj = null;
